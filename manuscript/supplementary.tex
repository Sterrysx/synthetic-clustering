...
concentrates on the diagonal: exact agreement rises from $68.2\%$ at
$\sigma = 0.1$ to $98.3\%$ at $\sigma = 10$ for $k$-means (Ward: $64.9\%$ to
$97.7\%$). In the overlapping regime the off-diagonal mass is not symmetric.
At $\sigma = 0.1$ the synthetic data yield a \emph{lower} estimate than the
original in $15.5\%$ of comparisons and a higher one in $17.1\%$; the largest
single off-diagonal cell is the original data supporting $\hat{k} \ge 5$ while
the synthetic data support $\hat{k} = 2$ ($8.2\%$). That is, when the original
...
\caption{Agreement in the estimated number of groups between original and
  ...
  agreement at $\sigma = 0.1$ is higher ($75.0\%$ for $k$-means, $68.0\%$ for
  Ward) while the remaining levels change by less than half a percentage point.
  $n = 18{,}000$ comparisons per separation level.}
\label{tabS:khat}
\small
